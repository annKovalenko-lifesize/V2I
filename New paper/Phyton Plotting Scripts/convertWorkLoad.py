...
#    tt=1 #temporary task type (just for checking)
    fd = open('/home/amini/Desktop/CSU/workspace/srm2/classes/classes_2', 'r')
    ETC.fill(0)
    for line in fd:
        if line.find("machine=") != -1:
            ...
            nominator+=float(probLine[1])
    avg=nominator/nominator #just for the very last item in the ETC
    ETC[prevTaskType,prevMachine]=avg
#    print ETC
...
if __name__ == '__main__':
    ...
    for i in range(1,NUM_TRIAL+1):
        ...
        for line in fd:
            ...
            taskType=int(parts[5].lstrip().rstrip())
...
#generating data size
            ...
            print str(dataSize)
            assmbledLine = parts[2].lstrip().rstrip() + ' ' + parts[5].lstrip().rstrip() +' '+str(deadline) +' '+str(dataSize)+';\n'
