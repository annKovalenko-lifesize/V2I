...
public class MobileDeviceManager extends DatacenterBroker {
	...
	protected void submitCloudlets() {
		...
		if(SimManager.getInstance().getEdgeOrchestrator().getPolicy() == "SJF") {
			
			SC_policy_Que = sortList;
		}
		else if(SimManager.getInstance().getEdgeOrchestrator().getPolicy() == "SLF") {
			SC_policy_Que = sortList;
		}			
		else {
		...
		for (Cloudlet cloudlet : sortList) {
			...
			schedule(getVmsToDatacentersMap().get(vm.getId()),cloudlet.getSubmissionTime(),CloudSimTags.CLOUDLET_SUBMIT, cloudlet);
		...
		for (Cloudlet cloudlet:pasuedCloudletList) {
			...
			System.out.println(" Cloudlet ID : " + cloudlet.getCloudletId()+ " Arrival time : " + cloudlet.getSubmissionTime());
